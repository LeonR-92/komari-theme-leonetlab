...
assert.match(header, /queuedThemeMode/)
...
assert.match(serviceWorker, /THEME_CACHE_PREFIXES/)
...
assert.match(bootHtml, /localStorage\.getItem\('appearance'\)/)
...
assert.match(bootHtml, /serviceWorker\.getRegistrations/)
assert.match(bootHtml, /registration\.unregister\(\)/)
