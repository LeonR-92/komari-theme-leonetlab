...
assert.match(app, /leonetlab:intro:1\.2\.7/)
...
// 交接卸载由 transitionend 驱动（固定计时器仅兜底），飞行前等待目标地球仪挂载。
...
// 交接朝向共享必须位于独立模块（模块级单例），且 reduced-motion 下停止自动旋转。
const globeIntroShared = read('src/utils/globeIntroShared.ts')
assert.match(globeIntroShared, /export const sharedIntroOrientation: GlobeOrientation = \{ phi: 0, theta: 0, valid: false \}/)
assert.match(earthGlobe, /from '@\/utils\/globeIntroShared'/)
assert.match(earthGlobe, /__lnlGlobeProbe|globeIntroShared/)
assert.match(earthGlobe, /adoptSharedIntroOrientation/)
...
assert.doesNotMatch(earthGlobe, /const sharedIntroOrientation = /)
...
assert.match(loadingCover, /remeasureHandoff/)
...
assert.match(smokeTest, /handoffPhiError/)
assert.match(smokeTest, /dashboardNoBackwardJump/)
